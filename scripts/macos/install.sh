...
require_macos
require_tools launchctl python3 shasum
...
<key>ProgramArguments</key><array><string>$HOST_EXECUTABLE</string><string>--channel</string><string>official</string></array>
<key>RunAtLoad</key><true/><key>KeepAlive</key><true/>
</dict></plist>
PLIST
chmod 644 "$LAUNCH_AGENT"
"$SCRIPT_DIR/start-host.sh"
echo "AIFRED Official installed from commit $(cat "$OUT_ROOT/commit.txt"). User settings retained."
