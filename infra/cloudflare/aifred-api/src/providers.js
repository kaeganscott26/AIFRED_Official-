...
export function providerConfiguration(env) {
  ...
  return {
    ...
    providers: [
      {
        id: "ollama",
        configured: Boolean(env.OLLAMA_BASE_URL && env.OLLAMA_MODEL && env.OLLAMA_API_TOKEN),
        model: bounded(env.OLLAMA_MODEL, 120),
...
export async function invokeChatProvider(env, body) {
  ...
  if (provider === "openai") {
    ...
    if (!env.OLLAMA_BASE_URL || !env.OLLAMA_MODEL || !env.OLLAMA_API_TOKEN) {
    ...
    model = requestedModel || bounded(env.OLLAMA_MODEL, 120);
  } else {
