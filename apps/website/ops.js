(() => {
  ...
  const parsedTime = (v) => {
    const d = new Date(v);
    return !v || Number.isNaN(d.valueOf()) ? null : d;
  };
  const time = (v) => {
    const d = parsedTime(v);
    return d
  ...
  const timeCell = (v) => {
    const d = parsedTime(v);
    return d
      ? `${escape(time(v))}<br><small class="muted" title="Raw UTC timestamp">UTC ${escape(d.toISOString())}</small>`
      : "—";
  };
  ...
  const card = (label, value, detail = "") =>
    `<article class="ops-card metric"><span>${escape(label)}</span><strong>${escape(value)}</strong><small class="muted">${escape(detail)}</small></article>`;
  ...
  const eventColumns = [
    {
      label: "Time",
      render: (r) => timeCell(r.timestamp || r.created_at || r.createdAt),
    },
    {
      label: "Event",
      render: (r) =>
        `<span class="event-type">${escape(r.event_type || r.type || "event")}</span>`,
    },
    {
      label: "Source",
      render: (r) =>
        escape(r.source || r.metadata?.source || r.referrer || "—"),
    },
    {
      label: "Details",
      render: (r) =>
  ...
  function render() {
    const d = state.dashboard || {},
      traffic = d.traffic || {};
    const events = state.logs.filter(
      (x) => !String(x.event_type || "").startsWith("admin."),
    );
    const downloads = events.filter((x) =>
      String(x.event_type || "").includes("download"),
    );
    const errors = events.filter((x) =>
      /error|fail/i.test(`${x.event_type || ""}${x.message || ""}`),
    );
    $("#overview").innerHTML =
    ...
    const counts = Object.entries(
      events.reduce((a, x) => {
        const name = x.event_type || "unknown";
        a[name] = (a[name] || 0) + 1;
        return a;
      }, {}),
    ).sort((a, b) => b[1] - a[1]);
    $("#analytics").innerHTML =
      `<h2>Analytics</h2><div class="metric-grid">${card("Events", traffic.api_hits || events.length, "Compact lifetime aggregate")}${card("Sessions", new Set(events.map((x) => x.session_id).filter(Boolean)).size, "Recent bounded identifiers")}${card("Downloads", traffic.downloads ?? "Unavailable", "Canonical download.counted only")}${card("Lifecycle events", downloads.length, "Recent diagnostics, not summed")}${card("Media streams", traffic.media_streams || 0, "Compact lifetime aggregate")}</div>${table(
        counts.map(([type, count]) => ({ type, count })),
        [
          { label: "Event type", key: "type" },
          { label: "Count", key: "count" },
        ],
      )}<p class="muted">Historical requested/completed/clicked events remain available but are excluded from the canonical download total because unique human delivery cannot be reconstructed reliably.</p>`;
    $("#downloads").innerHTML =
      `<h2>Downloads</h2>${table(downloads, eventColumns)}`;
    $("#track-analysis").innerHTML =
      `<h2>Track Analysis</h2><div class="metric-grid">${card("Catalog tracks", state.tracks.length, "Public catalog")}${card("Accepted analyses", state.references.length, "Reference pool")}</div><h3>Analysis records</h3>${table(state.references.slice(0, 100), eventColumns)}<h3>Catalog</h3>${table(
        state.tracks.slice(0, 100),
        [
          { label: "Track", render: (r) => escape(r.title || r.name || r.id) },
          { label: "Artist", render: (r) => escape(r.artist || "—") },
          { label: "Version", render: (r) => escape(r.version || "—") },
        ],
      )}`;
    $("#api").innerHTML =
      `<h2>API</h2><div class="metric-grid">${card("Production base", location.origin, "Canonical custom domain")}${card("Version", state.status?.api_version || "v1", "Normalized contract")}${card("Models", state.models?.models?.length || 0, state.models?.active_model || "")}</div><pre>${escape(JSON.stringify({ health: "/health", models: "/v1/models", chat: "/v1/chat/completions", admin: "/api/v1/admin/*", providers: state.models?.providers || {} }, null, 2))}</pre>`;
    $("#logs").innerHTML =
      `<h2>Logs / Errors</h2><div class="toolbar"><input id="log-filter" class="ops-control" placeholder="Filter current bounded log"></div><div id="log-table">${table(state.logs.slice(0, 300), eventColumns)}</div>`;
    $("#inquiries").innerHTML = `<h2>Inquiries</h2>${table(
      state.inquiries.slice(0, 200),
      [
        { label: "Time", render: (r) => timeCell(r.timestamp || r.created_at) },
        { label: "Name", render: (r) => escape(r.name || "—") },
        {
          label: "Contact",
          render: (r) => escape(r.email || r.contact || "—"),
        },
        { label: "Status", render: (r) => escape(r.status || "received") },
      ],
    )}`;
    $("#exports").innerHTML =
      '<h2>Exports</h2><div class="card-grid"><article class="ops-card"><h3>Site Data</h3><p>Sanitized analytics, downloads, sessions, inquiries, errors, activity and deployment state.</p><button data-export="site">Export Site Data</button></article><article class="ops-card"><h3>Track Analysis</h3><p>Catalog and authoritative accepted analysis/reference records.</p><button data-export="tracks">Export Track Analysis</button></article></div><p class="muted">Authenticated, UTC ISO-8601, and Cache-Control: no-store.</p>';
    $("#forge").innerHTML =
    ...
    $("#archive").innerHTML =
      '<h2>Archive</h2><article class="ops-card archive-note"><h3>Desktop-owned cold storage</h3><p>Permanent archives live under gitignored <code>runtime/aifred-archive</code>. Use Desktop Admin to view, verify, search, restore or manually prune them.</p><p>Rotation never removes Cloudflare production data.</p></article>';
    ...
    document
      ...
      .forEach((button) =>
        button.addEventListener("click", () =>
          downloadExport(button.dataset.export, button),
        ),
      );
  }
  ...
  async function downloadExport(kind, button) {
    ...
    note(`Preparing ${kind} export…`);
    try {
      const r = await request(`/api/v1/admin/export/${kind}`);
      if (!r.ok) throw new Error(`Export failed (${r.status})`);
      const blob = await r.blob(),
        disposition = r.headers.get("content-disposition") || "",
        filename =
          /filename="?([^";]+)"?/i.exec(disposition)?.[1] ||
          `aifred-${kind}-export.json`,
  ...
  function busy(active, label) {
    ...
    $("#connection").textContent =
      label || (auth() ? "Production connected" : "Signed out");
  }
  function signOut() {
  ...
  $("#login-form").addEventListener("submit", async (event) => {
    ...
    try {
      ...
      sessionStorage.setItem(key, body.session_token);
      $("#password").value = "";
      signedIn();
    } catch (error) {
