...
if (!branch || !/^[a-zA-Z0-9._/-]+$/.test(branch))
  throw new Error("Provide the Pages branch explicitly.");
for (const check of ["check-website-modules.mjs", "check-website-assets.mjs"]) {
  const result = spawnSync(
    process.execPath,
    [join(root, "apps", "website", "tools", check)],
...
for (const page of ["index.html", "ops.html"]) {
  const html = readFileSync(join(source, page), "utf8");
  for (const match of html.matchAll(
    /<(?:link|script)\b[^>]*(?:href|src)="([^"?#]+)(?:[?#][^"]*)?"/g,
  )) {
    const file = match[1].replace(/^\.?\//, "");
    if (!/^(?:https?:)?\/\//.test(match[1]) && !publicFiles.includes(file)) {
      throw new Error(
        `${page} references ${file}, which is missing from the Pages package`,
      );
    }
  }
}
...
await build({
  entryPoints: [join(source, "_worker.js")],
...
config = config.replace(
  'pages_build_output_dir = "apps/website"',
  'pages_build_output_dir = "AIFRED_Official-/apps/website"',
);
...
const changed =
  spawnSync("git", ["status", "--porcelain", "--", "apps/website"], {
    cwd: root,
    encoding: "utf8",
  }).stdout.length > 0;
const result = spawnSync(
  process.execPath,
  [
    join(root, "apps/node_modules/wrangler/bin/wrangler.js"),
    "pages",
    "deploy",
    output,
    "--cwd",
